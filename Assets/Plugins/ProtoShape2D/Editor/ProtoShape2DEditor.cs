...
public class PS2DEditor:Editor{
	...
	ProtoShape2D script;
	bool isDraggingPoint=false; //If the point is being dragged
	bool isDraggingAnything=false; //If point or handle is being dragged
	PS2DSnap snap=new PS2DSnap(); //For snap detection
	double lastClickTime=0; //For double-click detection
	Rect windowRect; //For point properties window
	bool dontDrawAddPoint=false; //For disabling + point marker when needed
	int markerID=0; //To hold ID of add point marker
	Plane objectPlane; //Mostly to position controls using plane's normal

	private string appVersionBase; //To keep base version of Unity ("2017.1")
	private Vector3 dragOrigin; //To keep the coordinates of the mouse cursor before you started to drag the point. Needed in older Unity versions
	...
	static void AddObject(PS2DType type){
		...
		go.GetComponent<ProtoShape2D>().SetSpriteMaterial((Material)Resources.Load("PS2DSpritesDefault"));
	...
	void Awake(){
		appVersionBase=Application.unityVersion.Substring(0,Application.unityVersion.LastIndexOf('.')); //Get base version of Unity editor: 5.1 or 2017.2 - removing the last part
		...
		if(script.spriteMaterial==null){
			script.spriteMaterial=(Material)Resources.Load("PS2DSpritesDefault");
		}
	...
	public void OnDestroy(){
		if (script==null){
			...
			if(script.fillType!=PS2DFillType.Color && script.fillType!=PS2DFillType.CustomMaterial){
				//DestroyImmediate(script.defaultMaterial);
			}
		}
	}
				...
				if(et==EventType.MouseDown && GUIUtility.hotControl!=0){
					...
					if(selPoint>-1){
						...
						if(Event.current.shift){
							SelectPoint(selPoint,!script.points[selPoint].selected);
						//If shift is now down we just set this point to selected
						}else{
							//If this point wasn't selected we deselect all other poings
							if(!script.points[selPoint].selected) DeselectAllPoints();
							SelectPoint(selPoint,true);
						}
					}
	...
	void DrawPoint(int pointID){
		...
		if(changed && !isDraggingPoint){
			...
			//If the dragging just started, we save the initial position
			dragOrigin=script.transform.TransformPoint(script.points[pointID].position);
			isDraggingPoint=true;
		//Modify position of a point that is being dragged to compensate for a bug in older Unity versions
		//This bug happens only on retina displays on mac
		}else if(changed && isDraggingPoint && (appVersionBase=="2017.2" || appVersionBase=="2017.1")){
			//When dragging continues, we multiply the distance of the dragging by a difference in DPI
		...
		//React to drag stop
		if(isDraggingPoint && et==EventType.MouseUp){
